...
(function () {
  ...
  const formatERC = (productType) => {
    ...
    if (productType.includes("3yr"))
      return "3% in year 1, 2% in year 2, 1% in year 3";
    return "Refer to product terms";
  };

  /* ------------------------------ Retention Logic ------------------------------ */
  function applyRetentionAdjustments(
    baseRate,
    feePct,
    procFee,
    propertyType,
    ...
    if (isRetention !== "Yes")
      return { adjRate: baseRate, adjFee: feePct, adjProc: procFee };
  ...
  const defaultLTV = rules.default?.[propertyType] ?? 75;
  ...
  // Get retention override (if any)
  let retentionOverride = null;
  if (isRetention === "Yes" && numericLtv) {
    retentionOverride = rules.retention?.[propertyType]?.[numericLtv];
  }

  // Get flat above comm override (if applicable)
  ...
  if (
    ...
    flatOverride = rules.flatAboveCommOverrides[tier];
  }

  // Combine applicable rules
  const applicable = [defaultLTV];
  if (retentionOverride != null) applicable.push(retentionOverride);
  if (flatOverride != null) applicable.push(flatOverride);

  const finalMaxLTV = Math.min(...applicable);

  return finalMaxLTV / 100;
}
  ...
  function App() {
    ...
    // Get current criteria based on property type
    ...
    // Initialize criteria state based on current property type
    const initializeCriteriaState = (criteriaConfig) => {
      const state = {};

      if (criteriaConfig?.propertyQuestions) {
        criteriaConfig.propertyQuestions.forEach((q) => {
          state[q.key] = q.options[0].label;
        });
      }

      if (criteriaConfig?.applicantQuestions) {
        criteriaConfig.applicantQuestions.forEach((q) => {
          state[q.key] = q.options[0].label;
        });
      }
    ...
    const flatAboveCommVal = criteria.flatAboveComm === "Yes";
    ...
    const tier = useMemo(() => {
      ...
      // Check property questions
      if (currentCriteria?.propertyQuestions) {
        currentCriteria.propertyQuestions.forEach((q) => {
          const answer = criteria[q.key];
          const option = q.options.find((opt) => opt.label === answer);
          if (option && option.tier > maxTier) {
            maxTier = option.tier;
          }
        });
      }

      // Check applicant questions
      if (currentCriteria?.applicantQuestions) {
        currentCriteria.applicantQuestions.forEach((q) => {
          const answer = criteria[q.key];
          const option = q.options.find((opt) => opt.label === answer);
          if (option && option.tier > maxTier) {
            maxTier = option.tier;
    ...
    const handleSendQuote = async () => {
      ...
      try {
        ...
        try {
          ...
          console.warn(
            "JSON POST failed (expected in browser due to CORS):",
            e
          );
        }
        ...
        console.error(
          "An unexpected error occurred in handleSendQuote:",
          error
        );
        setSendStatus("error");
      } finally {
        ...
        {canShowMatrix && (
          <div className="card" style={{ gridColumn: "1 / -1" }}>
            ...
            >
              ...
              maximum net loan, *unless manually overridden by the sliders or
              the rate field.*
            </div>
